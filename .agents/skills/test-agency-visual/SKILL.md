...
description: Hybrid integration testing for agency's TUI combining automated scenario execution with human visual checkpoints via Claude Desktop. Runs the same build, tmux, and state-inspection steps as test-agency, then pauses after Scenarios 1, 2, and 3 to request a screenshot review from Claude Desktop. Findings (automated + visual) are written to .claude/test-findings/<branch>-<sha>.md. Usage: /test-agency-visual [--plan path/to/plan.md] ["brief feature description"]
---

# test-agency-visual

Hybrid integration testing skill for agency. Drives the real TUI via tmux, uses Claude's judgment to evaluate each automated step, and adds three visual checkpoints where Claude Desktop reviews the terminal for layout and rendering issues. All findings are written to a single findings file.
...
**Invocation:** /test-agency-visual <args>

## Baseline Scenarios

### ✅ Launch
<one-line description of what was observed>
**Visual (Claude Desktop):** <relayed findings, or "skipped">
...
Include only sections that apply. Omit "Branch-Specific Scenarios" if no branch-specific scenarios were run. Visual checkpoint lines should always be included for Scenarios 1–3, even if skipped.
